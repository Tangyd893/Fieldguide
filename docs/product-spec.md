...
> 版本：v0.5 | 状态：愿景对齐（独立软件理解工作台；可组合分屏；默认不耦合外部 PKB，可显式绑定，F-17）
...
Fieldguide 默认不依赖 Obsidian 或其他外部个人知识库：知识与问题产物保存在本机 Fieldguide 数据中。用户可**显式绑定**一个 Obsidian vault，把项目拆解产物单向导出为卡片与索引（F-17），随时可解绑。

### 1.3 设计原则

1. **教，而非炫**：节点少而精、摘要可读、Tour 有叙事顺序。
2. **本地优先 / 产品独立**：索引、图谱、知识卡、面试题在用户机器；不与外部笔记应用耦合；唯一的可选例外是 F-17：用户**显式绑定**一个 Obsidian vault 做单向导出，随时可解绑。
...
- **刻意不做**：双向同步（Obsidian 侧改动只回读为上下文，不合并进 SQLite）、接管 Obsidian 自身配置（`obsidian.json` 等）、Dataview / Bases / 插件依赖、移动端、每项目独立 vault、每个图节点一条笔记；**可选扩展**为 F-17 单向导出（卡片 + 索引 MOC，CLI 门禁，见 §五）
...
| F-17 | Obsidian vault 联动（CLI 门禁 + 卡片/索引同步 + Agent 回读） | FG（官方 Obsidian CLI） | P1 | 6 |
