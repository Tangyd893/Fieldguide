      ...
      <div className="flex-1 overflow-auto text-sm">
        {loading ? <div className="p-4 text-gray-400 text-xs">{t('codeMap.loading')}</div>
          : filtered.length === 0 ? <div className="p-4 text-gray-400 text-xs">{t('codeMap.noFiles')}</div>
            : <TreeNodeList entries={filtered} depth={0} onFileClick={onFileClick} activeFilePath={activeFilePath} />}
      </div>
    </div>
  )
}

function TreeNodeList({ entries, depth, onFileClick, activeFilePath }: {
  entries: FileEntry[]; depth: number; onFileClick: (p: string) => void; activeFilePath?: string
}) {
  return <>{entries.map(e => <TreeNode key={e.path} entry={e} depth={depth} onFileClick={onFileClick} activeFilePath={activeFilePath} />)}</>
}

function TreeNode({ entry, depth, onFileClick, activeFilePath }: {
  entry: FileEntry; depth: number; onFileClick: (p: string) => void; activeFilePath?: string
}) {
  ...
  return (
    <div>
      ...
      {hasChildren && expanded && <TreeNodeList entries={entry.children!} depth={depth + 1} onFileClick={onFileClick} activeFilePath={activeFilePath} />}
    </div>
  )
}
